# 🚀 MITILENCE 2K26 – Event Management Platform

<div align="center">

![MITILENCE](https://img.shields.io/badge/Event-MITILENCE%202K26-blue)
![Status](https://img.shields.io/badge/Status-Live-success)
![Platform](https://img.shields.io/badge/Platform-Web-orange)
![License](https://img.shields.io/badge/License-MIT-green)

A modern event management platform built for the **MITILENCE 2K26 Robotics Symposium** organized by the **Department of Robotics & Automation – ROBOZEN Association**.

</div>

---

# 🌐 Live Website

👉 https://mitilencera.online

---

# 📌 Project Overview

MITILENCE 2K26 is a **college technical symposium website** designed to manage:

- Event details
- Participant registration
- Team management
- Paper submission
- Admin approval system
- Event gallery
- AI assistant support

This platform helps organizers **efficiently manage event participants and registrations**.

---

# 🎯 Features

### 🧾 Event Management
- Technical & Non-technical event listings
- Detailed rules and instructions
- Team size configuration

### 👥 Registration System
- Individual and team registration
- Paper upload (Google Drive integration)
- Offline payment model

### 📊 Admin Dashboard
- Approve / reject registrations
- Disqualify teams
- Manage participants
- View uploaded research papers

### 🖼 Gallery System
- Previous year event photos
- Current year event media

### 🤖 AI Chat Assistant
- Event guidance
- Participant support

### 📱 Responsive Design
- Mobile friendly
- Clean navigation
- Fast performance

---

# 🧠 Tech Stack

| Technology | Usage |
|-----------|------|
| Next.js | Frontend Framework |
| React | UI Components |
| TailwindCSS | Styling |
| Firebase | Authentication & Database |
| Cloudinary | Image Hosting |
| OpenRouter AI | Chatbot Integration |

---

# 🏆 Events Included

| Event | Category |
|------|------|
| Robo Race | Technical |
| Tug of Bot | Technical |
| Robo Soccer | Technical |
| Paper Presentation | Technical |
| IPL Auction | Non Technical |
| Stumble Guys | Non Technical |

---

# 🏫 Organization

**Manakula Vinayagar Institute of Technology**

Association: **ROBOZEN**

Department: **Robotics & Automation**

Event: **MITILENCE 2K26**

---

# 👨‍💻 Developed By
**G.V.V. Satyanarayana (Satish)**  
Robotics & Automation Department  

📧 Email  
ganisettisatish34@gmail.com

---

# 📸 Screenshots
**HOME PAGE**
<img width="1893" height="989" alt="Screenshot 2026-03-05 083211" src="https://github.com/user-attachments/assets/0d2a47f6-58ed-4426-b175-c42761a60b24" />

**EVENTS PAGE**
<img width="1889" height="978" alt="Screenshot 2026-03-05 083438" src="https://github.com/user-attachments/assets/0a1ee28a-d8a0-44a5-91c5-ab8faafae273" />

**GALLERY PAGE**
<img width="1894" height="994" alt="Screenshot 2026-03-05 083652" src="https://github.com/user-attachments/assets/30861f16-2fb3-4984-8f66-0df2f8f54ee0" />

**DASHBOARD PAGE**
<img width="1887" height="987" alt="Screenshot 2026-03-05 083849" src="https://github.com/user-attachments/assets/f49f2168-2673-4a3f-a77b-980ce263703a" />

**LOGIN PAGE**
<img width="1885" height="993" alt="Screenshot 2026-03-05 083914" src="https://github.com/user-attachments/assets/38efc6d0-582c-4ce5-bb13-5f528a3facd8" />

**AI**
<img width="480" height="781" alt="Screenshot 2026-03-05 083938" src="https://github.com/user-attachments/assets/9d80ce80-ff95-42df-84ae-c4e3d6078e19" />


